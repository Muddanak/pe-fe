...
fn main() {
    ...
    let success = File::open(&args.filename).unwrap_or_else(|_| panic!("Could not open file {}", &args.filename));
    let chunk :Vec<u8> = get_large_data_chunk(success);
    let dos_header_data = &chunk[0..=254];

    let mz_offset = check_for_mz(dos_header_data).unwrap_or_else(|_| { panic!("{}", NoMZinFile) });
